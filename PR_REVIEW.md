...
- Added multiple `exit(1)` statements in `src/main.c` explicitly when expected tokens are missing or invalid operations occur (e.g., division by zero, invalid unary minus on booleans, and undefined variables) rather than silently propagating `0`.
