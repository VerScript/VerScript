...
- Added explicit `exit(1)` statements when expected tokens are missing or invalid operations occur (e.g., division by zero, invalid unary minus on booleans, and undefined variables) rather than silently propagating `0`.
...
- **Denial-of-Service (DoS) Risk**: If this interpreter runs continuously (like as a background engine, in long-running services, or an embedded context), submitting a script with a single undefined variable or invalid unary operator will instantly crash the process. An attacker can exploit this for a trivial DoS.
